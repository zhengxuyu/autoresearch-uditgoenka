...
- User invokes `/autoresearch_learn`
...
**CRITICAL — BLOCKING PREREQUISITE:** If invoked without `--mode` or sufficient inline context, you MUST use `question` to gather config BEFORE proceeding to Phase 1.

**TOOL AVAILABILITY:** `question` may be a deferred tool. If calling it fails, use `ToolSearch` to fetch the schema first, then retry. NEVER skip setup because of tool issues.
...
### Questions (single question call — 4 questions)
...
4. **Strategy by count:**
   - **0 files:** Warn via question: "No docs found. Switch to init mode? [yes/cancel]". If yes → restart as init. If cancel → STOP.
...
/autoresearch_learn --mode update
/autoresearch_predict --scope src/**
...
/autoresearch_learn --mode update
/autoresearch_ship --type code-pr

# Full quality pipeline
/autoresearch_learn --mode init
/autoresearch_scenario --domain software
/autoresearch_security
/autoresearch_ship
```
