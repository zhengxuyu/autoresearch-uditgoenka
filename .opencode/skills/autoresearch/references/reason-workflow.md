...
- User invokes `/autoresearch_reason`
...
**CRITICAL — BLOCKING PREREQUISITE:** If `/autoresearch_reason` is invoked without task, domain, and mode all provided, you MUST use `question` to gather context BEFORE proceeding to Phase 1. DO NOT skip this step.

**TOOL AVAILABILITY:** `question` may be a deferred tool. If calling it fails, use `ToolSearch` to fetch the schema first, then retry.
...
Batch ALL selected questions into a SINGLE `question` call:
...
**STOP: Have you completed the Interactive Setup above?** Complete `question` before entering this phase.
